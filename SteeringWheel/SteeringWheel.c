...
void SteeringWheel_Deal(SteeringWheel *pSteWhe, float vel, float direction, float omega)
{
	...
	PID_Calculate(pSteWhe->currentDirection, pSteWhe->putoutDirection, &pSteWhe->Steering_Dir_PID); 												//角度环
  	PID_Calculate(pSteWhe->SteeringMotor.Speed, pSteWhe->Steering_Dir_PID.Output, &pSteWhe->Steering_Vel_PID);                         //速度环
}
