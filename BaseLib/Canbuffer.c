...
uint8_t CAN_Transmit(CAN_HandleTypeDef *hcan,CAN_TxHeaderTypeDef *head,uint8_t *buf)
{
    ...
    if (hcan->Instance==CAN1)
    {
        ...
        if (can1_send_point==CAN1_BUFFER_SIZE)  //指针走出了数据缓冲区地址，回到头部
            can1_send_point=0;
        if ((!(sfr&(CAN_TSR_RQCP0|CAN_TSR_RQCP1|CAN_TSR_RQCP2)))&&((sfr&CAN_TSR_TME0)&&(sfr&CAN_TSR_TME1)&&(sfr&CAN_TSR_TME2)))   //邮箱全空，并且中断请求标志位为0
...
static void CAN_SendCompleteServe(CAN_HandleTypeDef *hcan)
{
    ...
    if (hcan->Instance==CAN1)
    {
        while(can1_data_num)        //重复执行直到发送缓冲区为空或者所有CAN邮箱均占满
        {
            if((sfr&CAN_TSR_TME0)|(sfr&CAN_TSR_TME1)|(sfr&CAN_TSR_TME2))   //有空邮箱
                HAL_CAN_AddTxMessage(hcan,&((can1sendbuffer+can1_read_point)->head),(can1sendbuffer+can1_read_point)->buffer,&txMailbox);
            else
                break;
            can1_data_num=can1_data_num-1;
    ...
    else
    {
        ...
        {
            if((sfr&CAN_TSR_TME0)|(sfr&CAN_TSR_TME1)|(sfr&CAN_TSR_TME2))   //有空邮箱
                HAL_CAN_AddTxMessage(hcan,&((can2sendbuffer+can2_read_point)->head),(can2sendbuffer+can2_read_point)->buffer,&txMailbox);
            else
                break;
            can2_data_num=can2_data_num-1;
